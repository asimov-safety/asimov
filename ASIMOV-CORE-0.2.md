...
The first implementation milestone is a disposable reference target plus direct resource-observed tests for normal allowed work, mediation bypass, revocation, parent/child revocation, supervisor tampering and persistent human stop. It should intentionally fail when a control is removed. A second independently implemented target is needed before claiming broad portability. Runic is a proposed integration target only; its current code has not been examined for this draft.
